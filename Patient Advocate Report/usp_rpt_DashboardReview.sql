	  ...
	  FROM 
		(
			SELECT DISTINCT	AO.MRN
				 ...
				 , O.[Queue] AS [QUEUE]
				 , O.[Order Last Event] AS [Last Event]
				 , CAST(NULLIF(O.[last event date],'') AS DATE) AS [Last Event Date]
				 , CAST(O.[Ship Date] AS DATE) AS [Order Fill Date]				 
				 , CASE WHEN CAST(O.[Ship Date] AS DATE) > CAST(GETDATE() AS DATE) THEN CAST(O.[Ship Date] AS DATE)
					    ELSE CAST(O.[Next Fill Date] AS DATE)
				   END
				   AS [Next Fill]
				 --, IIF(
					--	[Ship Date] >= GETDate(),
			...
			SELECT DISTINCT PO.MRN
				 ...
				 --, IIF(
					...
					--[Next Fill Date]
					--) AS [Next Fill]
			     --, 'UNION 2' AS FromWhere
			  FROM dbo.[STG_DaybueOrders] O
			 INNER JOIN #003PendingOrders PO 
...
********************************************************/
	...
	********************************************************/
		...
		SELECT P.patient_full_name
		 ...
		 --SELECT mrn,COUNT(1) FROM #009Dashboard
		 -- GROUP BY mrn
		 -- HAVING COUNT(1) > 1		  
/*******************************************************
--------- 010 Precall Format -----------------
...
********************************************************/
	...
	*************************************************************/
	...
	IF(OBJECT_ID('dbo.rpt_100_Dashboard_Review') IS NOT NULL)
		TRUNCATE TABLE dbo.rpt_100_Dashboard_Review
	ELSE
	BEGIN
		CREATE TABLE dbo.rpt_100_Dashboard_Review(ID INT IDENTITY(1,1)				
				, [Name] VARCHAR(255), [MRN] BIGINT, [HUB ID] BIGINT,[Patient_Status] VARCHAR(255) ,[Order Status] VARCHAR(255),Payer VARCHAR(255) 
				, [Secondary] VARCHAR(255),[Queue] VARCHAR(255),[Last Event] VARCHAR(255),[Last Event Date] DATE,[Last Fill] DATE
				, [Next Fill] DATE,[Need by Date] DATE,Category VARCHAR(255),[Review Notes] VARCHAR(4000)
			)
	END

	INSERT INTO dbo.rpt_100_Dashboard_Review ([Name] , [MRN] , [HUB ID] ,[Patient_Status]  ,[Order Status] ,Payer  
				 , [Secondary] ,[Queue] ,[Last Event] ,[Last Event Date] ,[Last Fill]
