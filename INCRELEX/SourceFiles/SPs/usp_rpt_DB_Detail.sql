...
BEGIN
	...
	;WITH PendingOrders
	   AS
		(
		SELECT patient_mrn, order_status, order_rank, order_description
			 , CAST(NULLIF(ordered_date_timestamp,'') AS DATE) AS ordered_date_timestamp
			 , order_enterprise_list_code, order_enterprise_list_description
			 , order_discharge_date_timestamp, order_last_event, CAST(NULLIF(last_event_date,'') AS DATE) AS last_event_date 
			 , CAST(NULLIF(order_fill_date_timestamp,'') AS DATE) AS order_fill_date_timestamp
			 , CAST(NULLIF(order_next_fill_date_timestamp,'') AS DATE) AS order_next_fill_date_timestamp
		  FROM dbo.STG_Orders
		 WHERE (TRIM(order_status) = 'Pending')	--[0011 Pending Orders]		
		),
	ActiveOrders
	   AS
	   (
		SELECT AO.patient_mrn, CAST(Ao.[patient_mrn] AS BIGINT)-40119 AS [Patient ID], AO.order_status
			 , AO.order_rank, AO.order_description, CAST(NULLIF(AO.ordered_date_timestamp,'') AS DATE) AS ordered_date_timestamp
			 , AO.order_enterprise_list_code, AO.order_enterprise_list_description
			 , CAST(NULLIF(AO.order_discharge_date_timestamp,'') AS DATE) AS order_discharge_date_timestamp
			 , AO.order_last_event
			 , CAST(NULLIF(AO.last_event_date,'') AS DATE) AS last_event_date
			 , CAST(NULLIF(AO.order_fill_date_timestamp,'') AS DATE) AS order_fill_date_timestamp
			 , CAST(NULLIF(AO.order_next_fill_date_timestamp,'') AS DATE) AS order_next_fill_date_timestamp
		  FROM dbo.STG_Orders AO 
		  LEFT JOIN PendingOrders PO	--[0011 Pending Orders] 
			ON AO.patient_mrn = PO.patient_mrn
		 WHERE ((TRIM(AO.order_status)='Active') 
	...
	HAVING (MAX(AO.order_fill_date_timestamp) Is Not NULL AND (Max(AO.order_fill_date_timestamp)>CAST('1/1/2020' AS DATE)))
	...
	;WITH LastDispense
	   AS
		(
		SELECT mrn, Max(CAST(NULLIF(dispense_date,'') AS DATE) ) AS MaxOfdispense_date
	...
	DaySupply
	  AS
		(
		SELECT DISTINCT CAST(LD.[mrn] AS BIGINT)-40119 AS [Patient ID], LD.mrn, LD.MaxOfdispense_date
			 , Max(CAST(D.dispense_therapy_days AS INT) ) AS MaxOfdispense_therapy_days
		  FROM LastDispense LD 
		 INNER JOIN dbo.STG_Dispense D
		    ON LD.MaxOfdispense_date = CAST(D.dispense_date AS DATE)
		   AND LD.mrn = D.mrn
	...
	SELECT DISTINCT D.mrn, [mrn]-40119 AS [Patient ID]
		 ...
		 , CASE WHEN TRIM([line9]) Like '%Starter%' THEN 'Quick Start'
	...
	;WITH FirstDispenseType
	   AS
		(
		SELECT mrn, CAST([mrn] AS BIGINT)-40119 AS [Patient ID], Min(CAST(NULLIF(dispense_date,'') AS DATE)) AS MinOfdispense_date
		  FROM dbo.STG_Dispense
	...
	;WITH PrimaryPAApp
	   AS
		...
		SELECT note_patient_mrn, note_subject, CAST(NULLIF(note_entered_date,'') AS DATE) AS note_entered_date
		 ...
		 WHERE (TRIM(note_subject) Like '%Primary%') --146--[60 PrimaryPAApp]		
		)
	,MaxPrimaryPA
		AS
		(
	...
	SELECT CAST([MRN] AS BIGINT)-40119 AS [Patient ID], PPA.note_sys_id, PPA.note_patient_mrn AS MRN
	...
	SELECT CAST(MSPA.MRN AS BIGINT)-40119 AS [Patient ID], SPA.note_sys_id, SPA.note_patient_mrn AS MRN
	...
	SELECT mrn, CAST([mrn] AS BIGINT)-40119 AS [Patient ID], referral_source, referral_organization
		 ...
		 , CASE WHEN TRIM([referral_organization]) Like 'Walgreens%' THEN 'Walgreens Specialty'
				WHEN TRIM([referral_organization]) Like 'Accredo%' THEN 'Accredo'
				WHEN TRIM([referral_organization]) Like '%Optum%' THEN 'Optum'
				WHEN TRIM([referral_organization]) Like '%CVS%' THEN 'CVS'
				ELSE 'HCP'
	...
	SELECT S.[Patient ID]+40119 AS MRN, S.[Patient ID]
		 ...
		 , NULLIF(S.[Order Last Event Status],''), NULLIF(S.[Status Date],'')		 
		 , DATEDIFF(DAY,S.[Creation Date],GETDATE()) AS [Patient Aging Days]
		 , DATEDIFF(DAY,S.[status date],GETDATE()) AS [Status Aging Days]		 
		 , NULLIF(FD.MinOfdispense_date,'') AS [Initial Dispense Date]
		 , FDT.[Dispense Type] AS [Initial Dispense Type], NULLIF(S.[Last Dispense Date],'')
		 , DT.[Dispense Type] AS [Last Dispense Type]
		 , ED.MaxOfdispense_therapy_days AS [Therapy Days]
		 , NULLIF(ED.[Exhaust Date],'')
		 ...
		 , NULLIF(PPAS.[PA/Appeal Status Date],'')
		 ...
		 , SPAS.[Secondary PA/Appeal Status], NULLIF(SPAS.[Secondary PA/Appeal Status Date],'') 
	  FROM dbo.STG_100_Status_Output S
	  LEFT JOIN (
					SELECT mrn, CAST([mrn] AS BIGINT)-40119 AS [Patient ID]
	  ...
	  LEFT JOIN (
				SELECT mrn, CAST([mrn] AS BIGINT)-40119 AS [Patient ID], secondary_payer_type, secondary_payer, secondary_insurance_status
				  FROM dbo.STG_Patients
				 WHERE (TRIM(secondary_insurance_status)<>'Inactive')
				) SA
	...
	--SELECT * FROM dbo.STG_DB_Detail_Output -- 126
