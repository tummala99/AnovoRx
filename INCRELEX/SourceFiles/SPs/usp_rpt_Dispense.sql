...
BEGIN
	...
	SELECT DISTINCT D.[mrn]-40119 AS [Patient ID]
		 , CAST(NULLIF(D.order_original_rx_date,'') AS DATE) AS [Rx Date]
		 , CAST(NULLIF(D.dispense_date,'') AS DATE) AS [Dispensed Date]
		 ...
		 , IIF(TRIM(D.[line9]) LIKE '%Starter','Yes','') AS [Bridge Shipment]
		 , PP.[Patient Pay]
	FROM dbo.STG_Dispense D
	LEFT JOIN (
				SELECT DISTINCT patient_mrn
					 , CAST(NULLIF(ncpdp_date_filled_timestamp,'') AS DATE) AS ncpdp_date_filled_timestamp
	...
	LEFT JOIN dbo.STG_NCPDP NCP 
	 ...
	 AND (CAST(D.dispense_date AS DATE) = NCP.ncpdp_invoice_date_of_service_timestamp)
