...
BEGIN
	...
	SELECT O.patient_mrn, O.order_status, O.order_rank, O.order_description
		 , O.ordered_date_timestamp, O.order_enterprise_list_code, O.order_enterprise_list_description
		 , O.order_discharge_date_timestamp, O.order_last_event, O.last_event_date
		 , O.order_fill_date_timestamp, O.order_next_fill_date_timestamp
	...
	SELECT T.patient_mrn AS MRN, MAX(MaxOfordered_date_timestamp) AS StatusDate
	  ...
	  FROM 
		 (	--[0009 Combined Dates]
			SELECT O.patient_mrn, Max(O.ordered_date_timestamp) AS MaxOfordered_date_timestamp
			  FROM dbo.[STG_Orders] O
			 GROUP BY O.patient_mrn
			 UNION
			SELECT Q.mrn, Max(Q.move_date) AS MaxOfmove_date
			  FROM dbo.[STG_Queue] Q
			 GROUP BY Q.mrn
			 UNION
			SELECT O.patient_mrn, Max(O.last_event_date) AS MaxOflast_event_date
			  FROM dbo.[STG_Orders] O
			 GROUP BY O.patient_mrn
			HAVING (((Max(O.last_event_date)) Is Not Null))
			 UNION 
			SELECT D.mrn, Max(D.dispense_date) AS MaxOfdispense_date
	...
	SELECT note_patient_mrn, note_entered_date, note_subject, note_body
	  INTO #0016ClosedNotes
	  FROM dbo.[STG_Notes] 
	 WHERE (note_subject Like 'Patient Discharged%' Or (note_subject) Like 'No Start%') 
	...
	SELECT note_patient_mrn, Max(note_entered_date) AS MaxOfnote_entered_date
	...
	SELECT * 
	  ...
	  FROM
		 (
			SELECT DISTINCT A.patient_mrn
			 ...
			 INNER JOIN #0014UniqueOrders B
			  ...
			  LEFT JOIN dbo.STG_OrderLastEventMapping OLEM
				ON A.order_enterprise_list_description = OLEM.[Queue] 
			...
			SELECT CN.note_patient_mrn, CN.note_entered_date
			 ...
			 WHERE CN.note_patient_mrn NOT IN (229879,229745)
			   AND (CN.note_subject Like 'Patient Discharged%') 
			   AND (P.patient_status In ('Cancelled','Inactive'))
			...
			SELECT CN.note_patient_mrn, CN.note_entered_date
				 ...
				 , CASE WHEN [note_body] LIKE 'Patient unreachable%' THEN 'No Start: Patient Unreachable'
					    WHEN [note_body] LIKE 'Patient decision%' THEN 'No Start: Patient Decision'
						WHEN [note_body] LIKE 'copay%' THEN 'No Start: Copay too High'
			 ...
			 WHERE ((CN.note_patient_mrn<>231348) 
			   AND (CN.note_subject='No Start')
			   AND (P.patient_status IN ('Cancelled','Inactive','on hold')))
		) AS T
		...
		SELECT IIf(P.[mrn]=232086,'3/15/2022',P.[patient_referral_date]) AS [Creation Date]
			 ...
			 , DATEPART(YEAR,P.[patient_date_of_birth]) AS [Year of Birth]
		  ...
		  FROM dbo.STG_Patients P
		  LEFT JOIN (SELECT mrn, Max(dispense_date) AS MaxOfdispense_date
		...
		SELECT P.mrn, P.primary_payer_type AS [Payer Type], P.primary_payer AS Payer
			 ...
			 , IIf(SAP.[secondary_payer_type] IN ('Compassionate','Foundation','Copay Card'),'',P.[secondary_payer_type]) AS [Secondary Payer Type]
			 , IIf(SAP.[secondary_payer_type] IN ('Compassionate','Foundation','Copay Card'),'',P.[secondary_payer]) AS [Secondary Payer]
		  ...
		  LEFT JOIN (
					SELECT P.mrn, [mrn]-40119 AS ID, P.secondary_payer_type
					  ...
					  FROM dbo.STG_Patients P
					 WHERE (ISNULL(P.secondary_insurance_status,'Active')<>'Inactive')	
			 ) SAP
		    ON P.mrn = SAP.mrn;
		...
		SELECT N.note_patient_mrn AS mrn, N.note_subject
		 ...
		 WHERE ((N.note_subject Like 'Quick Start%') AND ((IIf([note_Subject] Like 'Quick Start Declined%' And [note_patient_mrn] Not In (235083),SUBSTRING([note_subject],23,LEN([note_subject])),null)) Is Not Null)) 
				OR (((N.note_subject) Like 'Quick Start%') AND ((IIf([note_subject]='Quick Start Accepted','Y',null)) Is Not Null));
		/****************0030 Optin**************************/
		...
		SELECT O.Mrn, O.[_2_did_anovo_receive_a_completed_copy_of_the_opt_in_letter_] AS OptIn
		  ...
		  LEFT JOIN dbo.STG_Notes N 
		   ...
		   AND (((N.note_subject) Like 'Pt Opt-Out%'))
		...
		AS
			(
			SELECT DISTINCT N.note_patient_mrn AS mrn, CAST(MIN(N.note_entered_date) AS DATE) AS [PA Approved]
			FROM dbo.STG_Notes N
			WHERE N.note_subject = 'prior auth approved'
			GROUP BY N.note_patient_mrn, N.note_subject	
			),
		MinOfdispense
		AS
			(
			SELECT DISTINCT D.mrn, CAST(MIN(D.dispense_date) AS DATE) AS MinOfdispense_date
			  FROM dbo.STG_Dispense D
		...
		SELECT O.patient_mrn, O.order_enterprise_list_description, O.order_fill_date_timestamp
			 , O.order_description
		  INTO #ScheduledShipDate
		  FROM dbo.STG_Orders O
		 WHERE ((O.order_enterprise_list_description Like 'C%') 
			OR (O.order_enterprise_list_description Like 'E%') 
			OR (O.order_enterprise_list_description Like 'D%'));
		...
		;WITH WecomeCallNotesAll --0050 Wecome Call Notes All
		  AS
		  (
			SELECT N.note_patient_mrn, N.note_sys_id, N.note_entered_date, N.note_subject
			  FROM dbo.STG_Notes N
		...
		SELECT [MRN]-40119 AS ID, A.note_sys_id, A.note_patient_mrn AS MRN
		  ...
		  FROM PrimaryPAApp A
		  LEFT JOIN MaxPrimaryPA B 
		    ON (B.MaxOfnote_sys_id = A.note_sys_id) 
		   AND (B.MRN = A.note_patient_mrn) 
		  LEFT JOIN dbo.STG_AuthNoteMapping C
		    ON A.note_subject = C.[Note Subject]
		 WHERE (((IIf(A.[note_subject] Like '%No Longer%',NULL,[MaxOfnote_entered_date])) Is Not Null))
		...
		SELECT [note_patient_mrn]-40119 AS ID
		  ...
		  LEFT JOIN MaxSecondaryPA B
		    ON B.MaxOfnote_sys_id = A.note_sys_id 
		   AND B.MRN = A.note_patient_mrn
		  LEFT JOIN dbo.STG_AuthNoteMapping C
		    ON A.note_subject = C.[Note Subject]
		 WHERE (((IIf(A.[note_subject] Like '%No Longer%',NULL,B.[MaxOfnote_entered_date])) Is Not Null))
		...
		SELECT DISTINCT PD.[Creation Date], PD.[mrn]-40119 AS [Patient ID], PD.[Patient Initials], PD.[Year of Birth]
			 , PD.[ICD-10] AS [ICD-10], SD.[Status Type], SD.[Order Last Event Status], SD.StatusDate AS [Status Date]
			 , SSD.order_fill_date_timestamp AS [Scheduled Ship Date], LD.MaxOfdispense_date AS [Last Dispense Date]
			 ...
			 , PPA.[PA/Appeal Status] AS [Primary PA/Appeal Status], PPA.[PA/Appeal Status Date] AS [Primary PA/Appeal Status Date]
			 , RP.[Secondary Payer Type], RP.[Secondary Payer], SPA.[Secondary PA/Appeal Status]
			 , SPA.[Secondary PA/Appeal Status Date], QSS.[Quick Start Shipment]
